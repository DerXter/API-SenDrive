        ...
        public static function afficheNavette($statut){
            ...
            if(!empty($statut)){
                if($statut != 'En cours' && $statut != 'Annulé' && $statut != 'Terminé'){
                    ...
                    $reqAfficheNavette = "SELECT DISTINCT n.idNavette, c.prenom AS prenom_client, c.nom AS nom_client, marque, modele, immatriculation, ca.prenom AS prenom_chauffeur, ca.nom AS nom_chauffeur, DATE_FORMAT(date, '%d/%m/%Y') AS dateDepart, heureDebut as heureDepart, heureFin as heureRetour, depart, destination, statut FROM Clientele c, Vehicule v, Chauffeur ca, Marque ma, Modele mo, Horaire h, Navette n WHERE v.idVehicule=n.idVehicule AND v.idMarque=ma.idMarque AND v.idModele=mo.idModele AND n.idClient=c.idClient AND n.idHoraire=h.idHoraire AND ca.idChauffeur=n.idChauffeur AND statut=?";
