            ...
            if(!empty($statut)){
                ...
                if($statut != 'En cours' && $statut != 'Annulé' && $statut != 'Terminé'){
                    ...
                    if($choix=='avec'){
                       ...
                       $reqAfficheNavette = "SELECT DISTINCT prenom AS prenom_client, nom AS nom_client, marque, modele, immatriculation, DATE_FORMAT(date, '%d/%m/%Y') AS date, heureDebut, heureFin, depart, destination, statut FROM Clientele c, Vehicule v, Marque ma, Modele mo, Horaire h, Navette n WHERE v.idVehicule=n.idVehicule AND v.idMarque=ma.idMarque AND v.idModele=mo.idModele AND n.idClient=c.idClient AND n.idHoraire=h.idHoraire AND n.idChauffeur IS NULL AND statut=?";
                    }
